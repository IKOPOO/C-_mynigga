...
int nomor; //variabel untuk menyimpan data no parkir yang ingin keluar dari parkiran 
vector <int> urutP; 
...
void inputAkhir(parkir &data){
  ...
  if(head == nullptr){
    ...
    current = head; //inisialisasi current itu sama dengah head
    while(current->next != nullptr){ // jika current ->next/ node selanjutnya itu tidak nullptr maka
      current = current->next; //data akan terus menunjuk kesamping sampai setelah node tersebut itu adalah nullptr
    }
    current->next = newnode; // jika sudah tercapai node terakhir atau yang selanjut dari node tersebut itu nullptr maka membuat node baru
  }
}
...
int cari_parkir(const string& noPlat, parkir &data){
  ...
  while(current != nullptr){
    if(current->noPlat == noPlat){
      ...
      urutP.insert(urutP.begin(), current->noParkir);  
      ditemukan = true; 
      break;
